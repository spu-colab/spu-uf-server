package br.gov.economia.seddm.spu.autodoc.model;
...
@Entity
@Table(name = "parametro", schema = "autodoc")
public class Parametro {
	...
	static Map<Integer, String[]> DESCRICOES = new LinkedHashMap<Integer, String[]>() {{ 
		...
		put(PASTA_DESTINO_PROCESSOS_SAPIENS, new String[] { "Pasta de destino dos processos do Sapiens (ap�s identifica��o dos munic�pios)", "" });
		put(ENDERECO_SEI, new String[] { "Endere�o do SEI", "https://sei.fazenda.gov.br/sei/inicializar.php" });
		put(ENDERECO_SAPIENS, new String[] { "Endere�o do Sapiens", "https://sapiens.agu.gov.br/login" });
		put(PASTA_DESPACHOS_SALVOS, new String[] { "Pasta de respostas impressas em PDF", "" });
		put(PASTA_PLANILHA_IMPORTACAO, new String[] { "Pasta da planilha de importa��o", "" });
		put(DEFAULT_BROWSER, new String[] { "Navegador padr�o", "Chrome" });
		put(ENDERECO_SPUNET, new String[] { "Endere�o do SPUNet", "http://spunet.planejamento.gov.br" });
		put(PASTA_ARQUIVOS_PROCESSOS_INDIVIDUAIS, new String[] { "Pasta de arquivos para anexar aos processos individuais", "" });
		put(UNIDADE_PADRAO_SEI, new String[] { "Unidade padr�o ao acessar o SEI", "SPU-MG-NUSUC" });
		put(BAIXAR_TODO_PROCESSO_SAPIENS, new String[] { "Baixar todo o processo do Sapiens", "N�o" });
		put(TEMPO_ESPERA, new String[] { "Tempo de espera para verifica��o (em segundos)", "0" });
	}};
